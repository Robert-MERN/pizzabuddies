...
const Food_menu_drawer = ({ drawer_state, toggle_drawer, sidebar, handle_sidebar, set_data }) => {
        ...
        >
            <div className='w-[90vw] md:w-[50vw] py-[10px]  text-stone-950 transition-all duration-300'>
                ...
                <div className='flex justify-between w-full items-center pt-[12px] pb-[30px] px-[20px]' >
                    <p className='text-[20px] font-semibold select-none' >Sidebar</p>
                ...
                {slider_menu_options.map((each, index) => (
                    <div
                        ...
                        key={index}
                        className={`py-[17px] px-[20px] border-b border-stone-200 cursor-pointer transition-all text-stone-700 ${sidebar === each.link ? "bg-stone-200 active:bg-gray-300 " : " active:bg-gray-100 "}`}
                    >
                        <button className=''>
                            <p className='text-[17px] font-medium select-none' >{each.option}</p>
                        </button>
                    </div>
                ))}
