...
const Checkouts_page = ({ axios, order_id }) => {
    ...
    const { confirm_order_api, set_API_loading, set_snackbar_alert, get_order_api } = useStateContext();
    ...
    useEffect(() => {
        if (order_id) {
            get_order_api(axios, order_id, set_order_details, set_confirmed_order, set_API_loading);
        }
    }, [order_id]);
            ...
            {confirmed_order ?
                ...
                <div className="flex-[1] lg:h-[calc(100vh-80px)] md:px-[40px] py-[40px] flex flex-col gap-4 lg:border-r border-stone-200 tracking-wider">
                    ...
                    <div className='p-4 border border-stone-200 rounded-md'>
                        ...
                        <p className='text-[16px] text-stone-700'>{confirmed_order.city} {confirmed_order.postal_code || ""}</p>
                        ...
                        <p className='text-[16px] text-stone-700'>Cash on Delivery (COD) • Rs. {calc_gross_total_amount(confirmed_order.purchase).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
